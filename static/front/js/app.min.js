...
function load_rand_post(t) {
	...
	$.ajax({url: theme.ajaxurl, type: "POST", dataType: "html", data: {action: t.action, data: t}}).done(function (a) {
...
function chack_name(t) {
	return !!RegExp(/[( )(\聽)(\~)(\!)(\@)(\#)(\$)(\%)(\^)(\*)(\()(\))(\+)(\=)(\[)(\])(\{)(\})(\\)(\;)(\:)(\')(\")(\,)(\.)(\/)(\<)(\>)(\禄)(\芦)(\鈥�)(\鈥�)(\?)(\)]+/).test(t)
}
...
function showAlert(t) {
	...
	switch (t.status) {
		case 0:
			localize.successAlert, e = "primary", a = "icon-loading icon-spin";
			break;
		case 1:
			localize.successAlert, e = "success", a = "icon-adopt";
			break;
		case 2:
			localize.infoAlert, e = "info", a = "icon-tishi";
			break;
		case 3:
			localize.warningAlert, e = "warning", a = "icon-warning";
			break;
		case 4:
			localize.errorAlert, e = "danger", a = "icon-close-circle"
	}
	var i = t.msg;
	...
	removeAlert(), $("#alert_placeholder").append(o), "primary" == e ? o.slideDown().addClass("show") : (o.slideDown().addClass("show"), setTimeout(function () {
		removeAlert(o)
	}, 3500))
}

function removeAlert(t) {
	t || (t = $(".io-alert-primary")), t[0] && (t.removeClass("show"), setTimeout(function () {
		t.remove()
	}, 300))
}
...
function ioConfirm(t, e, a) {
	var i = '\t<div class="io-bomb io-bomb-confirm io-bomb-open">\t\t<div class="io-bomb-overlay"></div>\t\t<div class="io-bomb-body">\t\t\t<div class="io-bomb-content bg-white text-sm">                <div class="io-bomb-header fx-yellow modal-header-bg text-center p-3">                    <i class="iconfont icon-tishi icon-2x"></i>                    <div class="text-md mt-1">' + t + '</div>                </div>\t\t\t\t<div class="m-4">' + e + '</div>                <div class="text-center mb-4">                    <button class="btn vc-red btn-shadow mx-2 px-5" onclick="_onclick(true);">' + localize.okBtn + '</button>                    <button class="btn vc-l-yellow btn-outline mx-2 px-5" onclick="_onclick(false);">' + localize.cancelBtn + "</button>                </div>\t\t\t</div>\t\t</div>\t</div>\t",
		o = $(i);
	$("body").addClass("modal-open").append(o), hasScrollbar() && $("body").css("padding-right", getScrollbarWidth()), _onclick = function (t) {
		n(), $.isFunction(a) && a(t, $(this))
	};
	var n = function () {
		$("body").removeClass("modal-open"), hasScrollbar() && $("body").css("padding-right", ""), $(o).removeClass("io-bomb-open").addClass("io-bomb-close"), setTimeout(function () {
...
function captcha_ajax(t) {
	...
	var o = t.data("action");
	if (o && (e.action = o), e.captcha_type && window.captcha && !window.captcha.ticket) return CaptchaOpen(t, e.captcha_type), !1;
	window.captcha && (e.captcha = JSON.parse(JSON.stringify(window.captcha)), e.captcha._this && delete e.captcha._this, window.captcha = {});
	var n = {};
	n.status = 0, n.msg = localize.loading, showAlert(n);
	var s = t.html();
	t.attr("disabled", !0).html('<i class="iconfont icon-loading icon-spin mr-2"></i>' + localize.wait), $.ajax({
		url: theme.ajaxurl,
		type: "POST",
		dataType: "json",
		data: e
	}).done(function (i) {
		i.msg ? (n.status = i.status, n.msg = i.msg, showAlert(n)) : removeAlert(), t.attr("disabled", !1).html(s), $.isFunction(a) && a(i, t, e), i.goto ? (window.location.href = i.goto, window.location.reload) : i.reload && window.location.reload()
	}).fail(function (e) {
		e = e.responseJSON, e && e.msg ? (n.status = e.status, n.msg = e.msg, showAlert(n)) : (n.status = 4, n.msg = localize.networkerror, showAlert(n)), t.attr("disabled", !1).html(s)
	})
}

function copyText(t, e, a, i) {
	var o = t.toString(), n = document.querySelector("#copy-input");
	n || (n = document.createElement("input"), n.id = "copy-input", n.readOnly = "readOnly", n.style.position = "fixed", n.style.left = "-2000px", n.style.zIndex = "-1000", i.parentNode.appendChild(n)), n.value = o, function (t, e, a) {
		if (t.createTextRange) {
			var i = t.createTextRange();
			i.collapse(!0), i.moveStart("character", e), i.moveEnd("character", a - e), i.select()
		} else t.setSelectionRange(e, a), t.select()
	}(n, 0, o.length), document.execCommand("copy") ? $.isFunction(e) && e() : $.isFunction(a) && a(), n.blur()
}
...
!function (t) {
	...
	function n() {
		try {
			...
			"0" === t || !t && window.matchMedia("(prefers-color-scheme: dark)").matches ? (document.documentElement.classList.add("io-black-mode"), document.documentElement.classList.remove(theme.defaultclass)) : (document.documentElement.classList.remove("io-black-mode"), document.documentElement.classList.add(theme.defaultclass)), o(!1)
		} catch (t) {
		}
	}
	...
	function l() {
		"1" == theme.minNav && !T && 767.98 < t(window).width() || !T && 767.98 < t(window).width() && t(window).width() < 1024 ? (t("#mini-button").prop("checked", !1), c(), T = !0, S && (t("body").addClass("mini-sidebar"), S = !1)) : "1" != theme.minNav && (T && t(window).width() >= 1024 || S && !T && t(window).width() >= 1024) ? (t("#mini-button").prop("checked", !0), c(), T = !1, S && (S = !1)) : t(window).width() < 767.98 && t("body").hasClass("mini-sidebar") && (t("body").removeClass("mini-sidebar"), S = !0, T = !1)
	}
	...
	function u(e, a, i) {
		e.addClass("disabled"), t.ajax({
			...
			i.trim() && (a.html(i), isPC() && t('.ajax-url [data-toggle="tooltip"]').tooltip({trigger: "hover"}), e.addClass("load")), e.removeClass("disabled")
		}).fail(function () {
	...
	function m(t) {
		for (var e = f("myLinks"), a = 0; a < e.length; a++) if (e[a].url == t.url) return void showAlert(JSON.parse('{"status":4,"msg":"' + localize.urlExist + '"}'));
		e.unshift(t), h(t, !1, !1), p(e, "myLinks")
	}
	...
	function h(e, a, i) {
		...
		a ? (i ? t(".my-click-list").prepend(s) : t(".my-click-list").append(s), s.children(".remove-site").on("click", g)) : (t("#add-site").before(s), s.children(".remove-site").on("click", b)), j && s.children(".remove-site").show(), isPC() && t('.new-site[data-toggle="tooltip"]').tooltip({trigger: "hover"})
	}
	...
	function v(e) {
		...
		if (a.length && !e && !t("#add-site")[0]) return void t(".customize_nothing.custom-site").children(".nothing").html('<a href="javascript:;" class="add-new-custom-site" data-action="add_custom_urls" data-term_name="鎴戠殑瀵艰埅" data-urls="' + Base64.encode(JSON.stringify(a)) + '" >鎮ㄥ凡鐧诲綍锛屾娴嬪埌鎮ㄧ殑璁惧涓婃湁鏁版嵁锛岀偣鍑�<strong style="color:#db2323">鍚屾鍒版湇鍔″櫒</strong>銆�</a>');
		if (a.length) for (var i = 0; i < a.length; i++) h(a[i], e, !1)
	}
	
	function b() {
		for (var e = t(this).data("id"), a = f("myLinks"), i = 0; i < a.length; i++) if (parseInt(a[i].id) === parseInt(e)) {
	...
	function g() {
		for (var e = t(this).data("id"), a = f("livelists"), i = 0; i < a.length; i++) if (parseInt(a[i].id) === parseInt(e)) {
			console.log(a[i].id, e), a.splice(i, 1);
			break
		}
	...
	t(document).ready(function () {
		if ("1" !== theme.minNav && l(), o(!1), y(), r(), isPC() ? t('[data-toggle="tooltip"]').tooltip({trigger: "hover"}) : t('.qr-img[data-toggle="tooltip"]').tooltip({trigger: "hover"}), s(), t(".sidebar-tools")[0] && t(".sidebar-tools").theiaStickySidebar({
			additionalMarginTop: 90,
			additionalMarginBottom: 20
		}), "1" === theme.isCustomize && (v(!1), v(!0)), t(window).scrollTop() >= 50 && (t("#go-to-up").fadeIn(200), t(".big-header-banner").addClass("header-bg")), t("#loading")[0]) {
			var i = t("#loading");
			i.addClass("close"), setTimeout(function () {
				i.remove()
			}, 600)
		}
		...
		}, 300), a(t(".sidebar-item.top-menu")), t(".io-ajax-auto").each(function () {
	...
	t(window).resize(function () {
		clearTimeout(_), _ = setTimeout(i, 200)
		...
		var e = t(this);
		if ("post_like" == e.data("action")) if (e.hasClass("liked")) showAlert(JSON.parse('{"status":1,"msg":"' + localize.liked + '"}')); else {
			var a = e.children(".flex-column");
			e.addClass("liked"), t.ajax({
				type: "POST",
				url: theme.ajaxurl,
				data: {action: e.data("action"), post_id: e.data("id"), ticket: e.data("ticket")},
				success: function (e) {
		...
		e.preventDefault(), t("html").toggleClass("io-black-mode " + theme.defaultclass), o(!0), t("#" + t(".switch-dark-mode").attr("aria-describedby")).remove()
	}), window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", n), t(window).scroll(function () {
	...
	t(document).on("click", ".sidebar-menu-inner .sidebar-more,.sidebar-menu-inner a", function () {
		...
		t(".sidebar-nav").hasClass() || t("body").addClass("animate-nav"), c()
		...
		window.location.href = theme.loginurl, window.location.reload
	}), t(document).on("click", ".ajax-list-home a:not(.load)", function (e) {
		e.preventDefault(), u(t(this), t(t(this).attr("href")).children(".ajax-list-body"), t(this).parents("li").data())
	...
	t(".customize-menu .btn-edit").click(function () {
		...
		if ("" !== i.url) {
			for (var o = f("livelists"), n = !0, s = 0; s < o.length; s++) o[s].name === i.name && (n = !1);
			if (n) {
				var r = theme.customizemax;
				o.length > r - 1 && (t(".my-click-list .site-" + o[r - 1].id).parent().remove(), o.splice(r - 1, 1)), h(i, !0, !0), o.unshift(i), p(o, "livelists")
			}
		}
	...
	t(".smart-tips.search-key").off().on({
		compositionstart: function () {
			...
			var e = t(this);
			if (A = "" != e.attr("zhannei"), z = e.parents("#search"), "true" == e.attr("data-status") && e.val() && !A) switch (theme.hotWords) {
				case"baidu":
					C(e.val(), z);
					break;
				case"google":
			...
			var a = t(this);
			if (A = "" != a.attr("zhannei"), z = a.parents("#search"), "true" == a.attr("data-status") && a.val()) {
			...
			if ("" == a.attr("zhannei")) {
				if (z = a.parents("#search"), 40 === e.keyCode) {
					O === $ - 1 ? O = 0 : O++, z.find(".search-smart-tips ul li").eq(O).addClass("current").siblings().removeClass("current");
					var i = z.find(".search-smart-tips ul li").eq(O).html();
					z.find(".smart-tips.search-key").val(i)
				}
				if (38 === e.keyCode) {
					e.preventDefault && e.preventDefault(), e.returnValue && (e.returnValue = !1), 0 === O || -1 === O ? O = $ - 1 : O--, z.find(".search-smart-tips ul li").eq(O).addClass("current").siblings().removeClass("current");
					var i = z.find(".search-smart-tips ul li").eq(O).html();
					z.find(".smart-tips.search-key").val(i)
				}
			}
	...
	t("#modal-new-url").on("blur", function () {
		var e = t(this);
		"" != e.val() && (isURL(e.val()) ? P != e.val() && (P = e.val(), x(e.val(), t(".add_new_sites_modal"))) : showAlert(JSON.parse('{"status":4,"msg":"URL 鏃犳晥锛�"}')))
		...
		var a = t(this), i = a.data("clipboard-text");
		a.hasClass("down_count") && t.ajax({
			type: "POST", url: theme.ajaxurl, data: a.data(), success: function (e) {
...
function ChromBookmarkConverter() {
	this.bookmarks = {folders: []}, this.stripUnneededTags = function (a) {
		...
		a = this.stripUnneededTags(a), c = $.parseHTML(a), $.each(c, function (a, c) {
			if ("DL" == c.tagName) {
				var d = {type: "folder", title: "鏈懡鍚�", items: []};
		...
		$.each(i.children(), function (a, i) {
			...
			e += 1, k = b + "." + e, 1 == j && i.tagName.toLowerCase() != "DL".toLowerCase() && (j = !1, console.log("h3", f), g.items.push(f)), i.tagName.toLowerCase() == "DL".toLowerCase() && (g = {
				...
				items: []
			}, 1 == j && (j = !1), d.bookmarks.folders.push(g), d.processDL(i, k, g)), i.tagName.toLowerCase() == "H3".toLowerCase() && (l = $(i), m = l.text() ? l.text() : "鏈懡鍚�", n = l.attr("add_date"), o = l.attr("last_modified"), f = {
				type: "header",
				title: m,
				add_date: n,
				last_modified: o
