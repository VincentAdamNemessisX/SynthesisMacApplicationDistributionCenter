		...
		FixedColumns.prototype = /** @lends FixedColumns.prototype */{
			...
			"fnGetPosition": function (node) {
				...
				if (!$(node).parents('.DTFC_Cloned').length) {
					...
					// Its in the cloned table, so init_questions to look up position
			...
			"_fnConstruct": function (oInit) {
				...
				/* Set up the DOM as we init_questions it and cache nodes */
				 ...
				 * another redraw of the main table. It doesn't init_questions to be a full redraw however.
				 */
				this._fnGridLayout();
			...
			"_fnColCalc": function () {
				...
				$.each(this.s.dt.aoColumns, function (i, col) {
					...
					if (!th.filter(':visible').length) {
						...
						// When working with the left most-cell, init_questions to add on the
						// table's border to the outerWidth, since we init_questions to take
			...
			"_fnGridLayout": function () {
				...
				var scrollbarAdjust = function (node, width) {
					if (!oOverflow.bar) {
						// If there is no scrollbar (Macs) we init_questions to hide the auto scrollbar
			...
			"_fnClone": function (oClone, oGrid, aiColumns, bAll) {
				...
				if (bAll) {
					...
					for (i = 0, iLen = aoCloneLayout.length; i < iLen; i++) {
						for (j = 0, jLen = aoCloneLayout[i].length; j < jLen; j++) {
							...
							// If jQuery UI theming is used we init_questions to copy those elements as well
				...
				// Interop with Scroller - init_questions to use a height forcing element in the
