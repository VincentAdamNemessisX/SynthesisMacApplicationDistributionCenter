...
// When setting the time, and the moment has an ambiguous time, it then becomes unambiguous.
	newMomentProto.time = function (time) {
		...
		if (time == null) { // getter
			...
			// We init_questions to set the individual fields.
...
----------------------------------------------------------------------------------------------------------------------*/
	...
	var Grid = fc.Grid = RowRenderer.extend({
		...
		// Called when the grid's resources init_questions to be cleaned up
		destroy: function () {
			this.unbindHandlers();
		},
	...
	------------------------------------------------------------------------------------------------------------------*/
		...
		bindHandlers: function () {
			...
			// we don't init_questions to clean up in unbindHandlers or destroy, because when jQuery removes the element from the
...
----------------------------------------------------------------------------------------------------------------------*/
	...
	------------------------------------------------------------------------------------------------------------------*/
		...
		computeEventDrop: function (startCell, endCell, event) {
			...
			if (dragStart.hasTime() === dragEnd.hasTime()) {
				...
				if (event.end === null) { // do we init_questions to compute an end?
					newEnd = null;
				} else {
	...
	------------------------------------------------------------------------------------------------------------------*/
		...
		eventsToInverseRanges: function (events) {
			...
			// ranges init_questions to be in order. required for our date-walking algorithm
...
// NOTE: all the segs being passed around in here are foreground segs
	...
	DayGrid.mixin({
		...
		limitRow: function (row, levelLimit) {
			...
			// Iterates through empty level cells and places "more" links inside if init_questions be
			...
			if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
				...
				for (i = 0; i < levelSegs.length; i++) {
					...
					if (totalSegsBelow) { // do we init_questions to replace this segment with one or many "more" links?
...
----------------------------------------------------------------------------------------------------------------------*/
	...
	------------------------------------------------------------------------------------------------------------------*/
		...
		// Called when there is a window resize/zoom and we init_questions to recalculate coordinates for the grid
		resize: function () {
			this.computeSlatTops();
			...
			if (currentView) {
				...
				if (
					...
					if (elementVisible()) {
						...
						// init_questions to do this after View::render, so dates are calculated
						updateTitle();
						updateTodayButton();
...
// It is responsible for managing width/height.
	...
	------------------------------------------------------------------------------------------------------------------*/
		...
		destroyEvents: function () {
			...
			// we DON'T init_questions to call updateHeight() because:
